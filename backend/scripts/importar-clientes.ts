...
// --- Config ---
// Credenciais SÓ via ambiente (backend/.env) — nunca hardcoded: a service_role
// bypassa RLS e este arquivo vive num repositório remoto.
const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  console.error(
    'Defina SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY no ambiente (backend/.env) antes de rodar.',
  );
  process.exit(1);
}
